...
- **Breaking:** `CombineDirectory,CombineFile` overloads that previously took a `PathFormat` now take a new `RelativePathFormat` enum (`MatchBase` / `Universal`) — the only valid options for an appended relative path. Callers that passed `PathFormat.Universal` should pass `RelativePathFormat.Universal`; callers that passed the base path's `PathFormat` should pass `RelativePathFormat.MatchBase` (or omit the argument)
- **Breaking:** `IAbsolutePath.PathExport` and `IPath.PathDisplay` now formalizes new path invariants: non-empty directory paths always end with the separator and file paths never do (previously, both types normalized to paths without any trailing separators)
...
- **Breaking:** Drop .NET Standard 2.1 - library now targets `net8.0+`
