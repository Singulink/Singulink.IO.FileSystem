...
> [!IMPORTANT]
> **`Singulink.IO.FileSystem` v3 contains breaking changes.** The `Combine{Directory,File}` overloads now take a `RelativePathFormat` enum (`MatchBase` / `Universal`) instead of a `PathFormat`, and non-directory paths always normalize with a trailing separator character. See the [change log](CHANGELOG.md) for full details.
